...
#define WND_CLASS_NAME TEXT("PowerPaimon")
...
void AddSystemTrayIcon(HWND hWnd)
{
	...
	lstrcpy(nid.szTip, TEXT("PowerPaimon"));
...
bool WriteConfig(std::string GamePath, int fps, std::string cmd)
{
...
void LoadConfig(int* defaultFPS, HWND* hWnd)
{
	...
	if (reader.ParseError() != 0)
	{
		...
		SendNotification(*hWnd, GetString(IDS_TOAST_CONFNOTFOUND_TITLE), GetString(IDS_TOAST_CONFNOTFOUND_MSG));
		...
		GamePath = szPath;
		CommandLine = "";
		WriteConfig(GamePath, *defaultFPS, CommandLine);
	...
	*defaultFPS = reader.GetInteger("Setting", "FPS", *defaultFPS);
	CommandLine = reader.Get("Setting", "CMD", "");
	...
	if (GetFileAttributesA(GamePath.c_str()) == INVALID_FILE_ATTRIBUTES)
	{
		...
		SendNotification(*hWnd, GetString(IDS_TOAST_MISSGAME_TITLE), GetString(IDS_TOAST_MISSGAME_MSG));
...
DWORD __stdcall _main(LPVOID p)
{
	...
	if (CommandLine == "-popupwindow")
		CheckMenuItem(hTaskTrayMenu, ID_BORDERLESS, MF_BYCOMMAND | MFS_CHECKED);

	std::string CommandLineArg = CommandLine;
	...
	if (!CreateProcessA(ProcessPath.c_str(), (LPSTR)CommandLineArg.c_str(), nullptr, nullptr, FALSE, 0, nullptr, nullptr, &si, &pi))
	{
		DWORD code = GetLastError();
...
LRESULT CALLBACK WindowProc(
	...
	switch (uMsg) {
		...
		case WM_COMMAND:
		{
			...
			case ID_BORDERLESS:
			{
				UINT uState = GetMenuState(hTaskTrayMenu, ID_BORDERLESS, MF_BYCOMMAND);
				if (uState & MFS_CHECKED)
				{
					CommandLine = "";
					WriteConfig(GamePath, SelectedFPS, CommandLine);
					CheckMenuItem(hTaskTrayMenu, ID_BORDERLESS, MF_BYCOMMAND | MFS_UNCHECKED);
					SendNotification(hWnd, GetString(IDS_TOAST_DISABLE_BORDERLESS_TITLE), GetString(IDS_TOAST_NEED_REBOOT_MSG));
				}
				else
				{
					...
					WriteConfig(GamePath, SelectedFPS, CommandLine);
					CheckMenuItem(hTaskTrayMenu, ID_BORDERLESS, MF_BYCOMMAND | MFS_CHECKED);
					SendNotification(hWnd, GetString(IDS_TOAST_ENABLE_BORDERLESS_TITLE), GetString(IDS_TOAST_NEED_REBOOT_MSG));
				}
				break;
			}
			case ID_ABOUT:
				ShellAbout(hWnd, "PowerPaimon", "v1.1.0", hIcon);
				break;
			case ID_QUIT:
				DestroyWindow(hWnd);
				break;
